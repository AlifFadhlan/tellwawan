...
import { getAllInterviews, getInterviewByUserId } from "@/data/admin";
import { currentUser } from "@/lib/auth";
import Link from "next/link";

import React from "react";

export const RekruterInterviewTables = async () => {
  const user = await currentUser();
  const id_user = user?.id;

  const interviews = await getInterviewByUserId(id_user);
